...
def _split_by_size(
    ...
    Ưu tiên cắt tại ranh giới paragraph (\n\n) thay vì giữa câu.
    """
    ...
    # Split theo paragraph trước, rồi ghép đến khi đủ chunk_chars
...
def get_embedding(text: str) -> List[float]:
    """
    Tạo embedding vector cho một đoạn text dùng OpenAI text-embedding-3-small.
    """
...
def build_index(docs_dir: Path = DOCS_DIR, db_dir: Path = CHROMA_DB_DIR) -> None:
    ...
    try:
        ...
        print("  Đã xóa collection cũ để rebuild.")
    except Exception:
        pass
    collection = client.get_or_create_collection(
        name="rag_lab",
