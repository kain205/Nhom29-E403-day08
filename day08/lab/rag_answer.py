...
def retrieve_hybrid(
    ...
    # Reciprocal Rank Fusion: RRF_score = sum(weight * 1/(60 + rank))
    doc_scores: Dict[str, Any] = {}

    for rank, chunk in enumerate(dense_results):
        key = chunk["text"][:120]
        if key not in doc_scores:
            doc_scores[key] = {"chunk": chunk, "rrf": 0.0}
        doc_scores[key]["rrf"] += dense_weight * (1.0 / (60 + rank))

    for rank, chunk in enumerate(sparse_results):
        key = chunk["text"][:120]
        if key not in doc_scores:
            doc_scores[key] = {"chunk": chunk, "rrf": 0.0}
        doc_scores[key]["rrf"] += sparse_weight * (1.0 / (60 + rank))

    sorted_docs = sorted(doc_scores.values(), key=lambda x: x["rrf"], reverse=True)[:top_k]
    results = []
...
VARIANT3_PROMPT_TEMPLATE = """\
# Role & Identity
You are an internal CS/IT Helpdesk assistant. Your purpose is to answer policy and support questions using only the provided documents.
...
- End with a Sources line listing all cited numbers, e.g. Sources: [1], [2]
...
def build_grounded_prompt(query: str, context_block: str, version: str = "v3") -> str:
    ...
    return VARIANT3_PROMPT_TEMPLATE.format(query=query, context_block=context_block)
